...
namespace SurvivalShooter
{
    public class SurvivalShooterMainEntry
    {
        ...
        internal static readonly API API = new API(ConstKey.CONST_PROJECT_NAME);
        ...
        {
            //Load environment 
            API.LoadGameObject(ConstKey.CONST_PROJECT_NAME, _env =>
            {
                environmentGameObject =UnityEngine.Object.Instantiate(_env);
                contentTrans = environmentGameObject.transform;
            });

            //load canvas group
            API.LoadGameObject(ConstKey.CONST_CANVAS_GROUP, _canvasPrefab =>
            {
                canvas =UnityEngine.Object.Instantiate(_canvasPrefab);
            ...
            //Load focus group
            API.LoadGameObject(ConstKey.CONST_FOCUS_GROUP, _focusGroup =>
            {
                focusGameObject = UnityEngine.Object.Instantiate(_focusGroup,GameObject.Find("AR Session Origin").transform);
                focusGameObject.SetActive(false);
            ...
            //Load audio clips
            API.LoadAudioClip(ConstKey.CONST_ZOMBUNNY_HURT_AUDIO,
                _zomBunnyHurt => bunyHurt = _zomBunnyHurt);

            API.LoadAudioClip(ConstKey.CONST_ZOMBUNNY_DEAD_AUDIO,
                _zomBunnyDead => bunyDead = _zomBunnyDead);

            API.LoadAudioClip(ConstKey.CONST_ZOMBEAR_HURT_AUDIO,
                _zomBearHurt => bearHurt = _zomBearHurt);

            API.LoadAudioClip(ConstKey.CONST_ZOMBEAR_DEAD_AUDIO,
                _zomBearDead => bearDead = _zomBearDead);

            API.LoadAudioClip(ConstKey.CONST_HELLEPHANT_HURT_AUDIO,
                _hellephantHurt => hellephantHurt = _hellephantHurt);

            API.LoadAudioClip(ConstKey.CONST_HELLEPHANT_DEAD_AUDIO,
                _zomHellephantDead => hellephantDead = _zomHellephantDead);

            API.LoadAudioClip(ConstKey.CONST_PLAYER_DEAD_AUDIO,
                _playerDead => playerDeathClip = _playerDead);

            API.LoadAudioClip(ConstKey.CONST_PLAYER_HURT_AUDIO,
                _playerHurt => playerHurtClip = _playerHurt);

            //Load Enemies prefab
            foreach (string tmp_EnemyName in ConstKey.EnemyNames)
            {
                API.LoadGameObject(tmp_EnemyName,
